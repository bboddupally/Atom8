...
<xsl:variable name="props" select="document('properties.xml')" />
        ...
        <xsl:result-document href="{testng:absolutePath('style.css')}" format="text">
            ...
            <xsl:choose>
                <xsl:when test="testng:isFilterSelected('FAIL') = 'true'">
                    .testMethodStatusFAIL { background-color: #ff4500; }
                </xsl:when>
                <xsl:otherwise>
                    .testMethodStatusFAIL { background-color: #ff4500; display: none; }
                </xsl:otherwise>
            </xsl:choose>

            <xsl:choose>
                <xsl:when test="testng:isFilterSelected('PASS') = 'true'">
                    .testMethodStatusPASS { background-color: #c2fb4e; }
                </xsl:when>
                <xsl:otherwise>
                    .testMethodStatusPASS { background-color: #c2fb4e; display: none; }
                </xsl:otherwise>
            </xsl:choose>
 ...
 <xsl:function name="f:getProperty">
         <xsl:param name="key" />
        <xsl:value-of select="substring-before(substring-after($props,concat($key,'=')),'&#xA;')" />
    </xsl:function>
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('reporterOutput.html')}" format="html">
            <html xmlns="http://www.w3.org/1999/xhtml">
                <xsl:call-template name="htmlHead"/>
                <body>
                    <h2><xsl:value-of select="f:getProperty('reports-screenShot')"/></h2>
        ...
        <xsl:result-document href="{testng:absolutePath('navigation.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <body>
                    ...
                    <xsl:value-of select="f:getProperty('reports-header')"/>
<!--                         <xsl:value-of select="testng:getVariableSafe($testNgXslt.reportTitle, 'GAME Smoke suite Results')"/>
 -->                    </h2>
                    <div>
                        <a href="overview.html" target="content"
                           onclick="javscript:clearAllSelections();">
                           <xsl:value-of select="f:getProperty('reports-overview')"/>
                    ...
                    <div>
                        <a href="reporterOutput.html" target="content"
                           onclick="javscript:clearAllSelections();">
                           <xsl:value-of select="f:getProperty('reports-screenShot')"/>
                        </a>
                    </div>
                     <div>
                        <a href="emailable-report.html" target="content"
                           onclick="javscript:clearAllSelections();">
                           <xsl:value-of select="f:getProperty('reports-failedlogs')"/>
                            ...
                            <xsl:choose>
                                <xsl:when test="exception/message">
                                    <pre style="padding: 5px; margin: 0;">
                                        <xsl:value-of select="testng:trim(exception/message)"/>
                                    </pre>
                                </xsl:when>
    ...
    <xsl:template name="percentageOverview">
        ...
        <td style="background-color: #ff4500; padding: 3px 3px 3px 0;" align="center">
            <div style="width: 50px;">
                <xsl:value-of select="$failedCount"/>
            </div>
        </td>
        <td style="background-color: #c2fb4e; padding: 3px 3px 3px 0;" align="center">
            <div style="width: 50px;">
    ...
    <xsl:template name="powered-by">
        <div style="margin-top: 15px; color: gray; text-align: center; font-size: 9px;">
            POWERED-BY
            <a href="http://gtech/QMS/processes/Quality%20Systems/The%20Team/Quality%20Systems%20Team.aspx" style="color: #8888aa;" target="_blank">
                GAME QA AUTOMATION TEAM
            </a>
        </div>
    </xsl:template>
